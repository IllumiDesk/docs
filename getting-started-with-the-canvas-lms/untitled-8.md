...
# Step 3: Access the Grader Console
...
* IllumiDesk application installed and `Developer Key --> Client ID` available, for example, `125900000000000042`
* IllumiDesk installed within your course
* Users within the Canvas LMS course.
* \(Optional\) If you would like to use the `Auto Grading` feature, then have at least two users within the course, one with the `Instructor` role and one with the `Learner` role.

{% hint style="success" %}
#### Multiple Graders

As of version 2.0, IllumiDesk now supports multiple graders for multiple courses. Previously, only one instructor could complete all grading tasks for a course. Multiple graders open the possibility to have other course members help with grading tasks, such as Teachers Assistants.
{% endhint %}
...
With the shared grader notebook open, you now have the option to add your Jupyter Notebook solution files prepared for grading. Now click on the `Formgrader` tab to access the interface which you can use to complete your grading tasks:  


![Shared grader notebook with Formgrader tab.](../.gitbook/assets/0035.png)

Clicking on the Formgrader tab will open a new browser tab. This new browser tab will display the options available to you to start managing your assignment files:
